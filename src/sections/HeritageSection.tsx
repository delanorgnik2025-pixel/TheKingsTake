...
function createMapPopup(
  ...
  popupNode.innerHTML = `
    <div style="background:rgba(21,32,43,0.95);backdrop-filter:blur(12px);border:1px solid rgba(255,149,0,0.3);border-radius:14px;padding:14px 18px;min-width:300px;max-width:400px;box-shadow:0 10px 40px rgba(0,0,0,0.6);font-family:'Newsreader',Georgia,serif;">
      ...
      <h3 style="font-size:18px;color:#F0EBE1;margin:0 0 6px 0;font-weight:500;">${territory.name}</h3>
      <p style="font-size:13px;color:#C9B99A;margin:0 0 12px 0;line-height:1.5;">${territory.description}</p>
      <div style="display:flex;flex-wrap:wrap;gap:4px;margin-bottom:10px;">
        ${territory.nations.slice(0, 6).map((n: string) => `<span style="font-size:11px;color:#FFB840;background:rgba(255,149,0,0.1);border-radius:5px;padding:3px 8px;">${n}</span>`).join('')}
...
const COUNTRY_MARKERS: Record<string, { name: string; coords: [number, number]; nations: typeof jamaicaNations }> = {
  ...
  cuba: { name: 'Cuba', coords: [-79.0, 21.5], nations: caribbeanNations.filter(n => n.country === 'Cuba') },
    ...
    import('mapbox-gl').then((mb) => {
      ...
      map.on('load', () => {
        ...
        ALL_TERRITORIES.forEach((territory) => {
          ...
          markerEl.addEventListener('mouseenter', () => { dot.style.transform = 'scale(1.5)'; dot.style.boxShadow = '0 0 16px rgba(255,149,0,0.8), 0 0 30px rgba(255,149,0,0.4)'; })
        ...
        map.on('click', (e: any) => {
          ...
          for (const t of ALL_TERRITORIES) {
            ...
            if (dist < 2.0) { setFocusedTerritory(t.id); map.flyTo({ center: t.coords, zoom: t.zoom, duration: 2000 }); return }
          }
